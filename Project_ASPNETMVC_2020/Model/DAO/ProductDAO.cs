...
namespace Project_ASPNETMVC_2020.Model.DAO
{
    public class ProductDAO
    {
        ...
        {
            return dBModel.products.Where(x => x.MEMORY == id).Count();
        }
        ...
        public IQueryable<product> productByPriceQuery(int id)
        {
            if (id == 1)
            {
                return dBModel.products.Where(x => x.PRICE.Value < 5000000);
            }
            else if (id == 2)
            {
                return dBModel.products.Where(x => x.PRICE.Value > 5000000 && x.PRICE.Value < 10000000);
            }
            else if (id == 3)
            {
                return dBModel.products.Where(x => x.PRICE.Value < 15000000 && x.PRICE.Value > 10000000);
            }
            else if (id == 4)
            {
                return dBModel.products.Where(x => x.PRICE.Value > 15000000 && x.PRICE.Value < 20000000);

            }
            else if (id == 5)
            {
                return dBModel.products.Where(x => x.PRICE.Value > 20000000);
            }
            else
            {
                return null;
